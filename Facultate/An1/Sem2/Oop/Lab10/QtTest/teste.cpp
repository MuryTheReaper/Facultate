...
void test_contract() {
  ...
  try {
    serv.add_to_contract("x", "1", ".", ".");assert(false);
  } catch (const contractException &ex) {
    std::string x = ex.getMsg();
    assert(x.find("Exista deja disciplina: x") >= 0);
...
void test_addrandom(){
  ...
  try {
    serv.addrandom("xy");assert(false);
