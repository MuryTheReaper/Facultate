...
vectorDinamic* filtrareCon(vectorDinamic* pharma, int conMin){
    /*
        Filtreaza farmacia dupa concentratia medicamentului.

        Args:
            farm* pharma : adresa catre farmacia in care se efectueaza modificarea
            int conMin  : marimea minima a concentratiei dupa care se face filtrarea
    ...
    */
    ...
    Iterator* i = creaazaIterator(pharma);
    for(prim(i);valid(i);urmator(i)){
        if(elementIT(i)->med.concentr >= conMin){
            pushBack(aux, *elementIT(i));
        }
    }
    distrugeIT(i);
