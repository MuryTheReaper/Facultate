...
void testCreeaza() {
  ...
  IteratorColectie ic = c.iterator(); // iterator pe colectia vida ar trebui sa
                                      // fie invalid din start
...
void testAdauga() {
  ...
  assert(c.vida() == false);
  assert(c.dim() == 10);
  for (int i = -10; i < 20; i++) { // mai adaugam elemente [-10, 20), acum
                                   // anumite elemente [0, 10) sunt de 2 ori
    c.adauga(i);
  }
  assert(c.vida() == false);
  assert(c.dim() == 40);
  for (int i = -100; i < 100;
       i++) { // mai adaugam elemente [-100, 100), acum anumite elemente [0, 10)
              // sunt de 3 ori, altele [-10, 0), si [10, 20) sunt de 2 ori
    c.adauga(i);
  }
  assert(c.vida() == false);
  assert(c.dim() == 240);
  for (int i = -200; i < 200; i++) { // numaram de cate ori apar anumite
                                     // elemente (inclusiv elemente inexistente)
  ...
  for (int i = 10000; i > -10000; i--) { // adaugam mult, si acum prima data
                                         // adaugam valori mari, dupa aceea mici
    c.adauga(i);
  }
  assert(c.dim() == 20240);
}
...
void testSterge() {
  ...
  assert(c.dim() == 0);
  for (int i = -100; i < 100;
       i = i + 2) { // adaugam elemente din 2 in 2 (numere pare)
    c.adauga(i);
  }
  for (int i = -100; i < 100;
       i++) { // stergem tot (inclusiv elemente inexistente)
  ...
  for (int i = 100; i > -100; i--) { // stergem descrescator (in ordine inversa
                                     // fata de ordinea adaugarii)
    if (i % 2 == 0) {
      assert(c.sterge(i) == true);
    } else {
      assert(c.sterge(i) == false);
  ...
  for (int i = -200; i < 200;
       i++) { // stergem 5 aparitii de la elemente inexistente si existente (dar
              // si acolo exista doar 4 aparitii)
    if (i < -100 || i >= 100) {
      assert(c.sterge(i) == false);
      assert(c.sterge(i) == false);
      assert(c.sterge(i) == false);
...
void testIterator() { // nu stim reprezentarea Colectiei, putem testa doar
                      // anumite lucruri generale, nu stim in ce ordine vor fi
                      // afisate elementele.
  Colectie c;
  IteratorColectie ic = c.iterator(); // iterator pe colectie vida
  assert(ic.valid() == false);

  for (int i = 0; i < 100; i++) { // adaug 100 de elemente, fiecare e valoarea
                                  // 33
    c.adauga(33);
  }
  IteratorColectie ic2 =
      c.iterator(); // daca iterez doar 33 poate sa-mi dea iteratorul
  ...
  assert(ic3.valid() ==
         true); // nu avem garantia ca elementele afisate vor fi egale, (adica
                // ca vom avea acelasi element de 3 ori consecutiv), testam doar
                // ca sunt 600 de elemente
  for (int i = 0; i < 600; i++) {
  ...
  int count = 0;
  while (ic4.valid()) { // fiecare element e multiplu de 4 si sunt in total 50
                        // de elemente
    TElem e = ic4.element();
    assert(e % 4 == 0);
    ic4.urmator();
  ...
  for (unsigned int i = 0; i < elemente.size();
       i++) { // scoatem pe rand elemente din vectorul dat de iterator,
              // verificam sa fie in colectie si le stergem si din vector si din
              // c4. Incepem stergerea cu ultimul
  ...
  for (unsigned int i = 0; i < elemente2.size();
       i++) { // scoatem pe rand elemente din vectorul dat de iterator,
              // verificam sa fie in colectie si le stergem si din vector si din
              // c4. Incepem stergerea cu primul
...
void testQuantity() { // scopul e sa adaugam multe date
  ...
  ic.prim();
  while (ic.valid()) { // fiecare element returnat de iterator trebuie sa fie in
                       // colectie si nrAparitii trebuie sa fie mai mare ca 0
    TElem e = ic.element();
  ...
  assert(ic.valid() == false);
  for (int i = 0; i < 10;
       i++) { // stergem multe elemente existente si inexistente
...
void test_functionalitate() {
  ...
  assert(c.dim() == 5);
  assert(c.elementeCuFrecventaData(2) == 2);
  try {
    c.elementeCuFrecventaData(-1);
    assert(false);
  } catch (exception) {
    assert(true);
  }
